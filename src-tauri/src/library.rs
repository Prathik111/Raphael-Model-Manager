...
pub(crate) async fn set_model_description(
    ...
    app.registry
        .update_model(
            ...
            serde_json::json!({ "description": if description.is_empty() { serde_json::Value::Null } else { serde_json::Value::String(description.clone()) } }),
        )
        .await?;
    ...
    c.execute(
        ...
        rusqlite::params![id, if description.is_empty() { None::<String> } else { Some(description) }, now()],
    )?;
    drop(c);
