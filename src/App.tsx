...
function App() {
  ...
  const doImport = async()=>{ if(!importUrl.trim()) return; setBusy(true); setImportError(null); try { const result=await api.importCivitai(importUrl.trim()); setPreview(result); setDownloadPath(result.target_directory); } catch (e) { setImportError(String(e)); } finally {setBusy(false);} };
  const install = async()=>{ if(!importUrl.trim()) return; setBusy(true); setImportError(null); try { const m=await api.installCivitai(importUrl.trim(),downloadPath); setPreview(null); setImportUrl(''); setDownloadPath(''); await refresh(); setSelectedId(m.id); } catch (e) { setImportError(String(e)); } finally { setBusy(false);} };
