...
function App() {
  ...
  return <div className={`app-shell thumb-fit-${thumbnailFit}`}><Background/><div className="noise"/>
    ...
    <div className="workspace">
      ...
      {selected && <Inspector key={selected.id} model={selected} images={images} allTags={allTags} galleryHasMore={galleryHasMore} galleryFetchBusy={galleryFetchBusy} onFetchMore={onFetchMore} onOpenImage={openImageViewer} onRefresh={async()=>{await api.refreshModel(selected.id); await refresh(); void (async()=>{try{const more=await api.syncModelGallery(selected.id,20); const result=await api.getImages(selected.id,1000); if(selectedIdRef.current===selected.id){setImages(result.images);setGalleryHasMore(more);}}catch{}})();}} onLinkCivitai={async(url)=>{await api.linkModelCivitai(selected.id,url); await refresh(); void (async()=>{try{const more=await api.syncModelGallery(selected.id,20); const result=await api.getImages(selected.id,1000); if(selectedIdRef.current===selected.id){setImages(result.images);setGalleryHasMore(more);}}catch{}})();}} onSaveTags={async(tags)=>{await api.setModelTags(selected.id,tags); await refresh();}} onSaveType={async(nextType)=>{await api.setModelType(selected.id,nextType); await refresh();}} onDelete={async()=>{await api.deleteModel(selected.id); setSelectedId(null); await refresh();}} onFilterTag={tag=>{setActiveTags(current=>current.some(x=>x.toLowerCase()===tag.toLowerCase())?current:[...current,tag]);}} onChangeCover={()=>setCoverEditorOpen(true)} onChooseThumbnail={async imageId=>{const updated=await api.setModelCoverFromImage(selected.id,imageId); setModels(current=>current.map(item=>item.id===updated.id?updated:item));}}/>}
