...
import type { AppState, CivitaiImportPreview, ModelImage, ModelRecord, ModelType } from './types';
...
function Inspector({ model, images, onRefresh }: { model: ModelRecord; images: ModelImage[]; onRefresh: ()=>void }) {
  ...
  return <aside className="inspector hud-panel">
    ...
    {tab==='overview' && <div className="inspector-scroll">
      ...
      {model.civitai_url && <section><div className="section-head">CIVITAI</div><div className="mono-box">{model.civitai_url}</div><button className="text-btn" onClick={onRefresh}>REFRESH CIVITAI DATA</button></section>}
...
function App() {
  ...
  const [type,setType]=useState<ModelType|'All'>('All'); const [query,setQuery]=useState(''); const [images,setImages]=useState<ModelImage[]>([]); const [importUrl,setImportUrl]=useState(''); const [preview,setPreview]=useState<CivitaiImportPreview|null>(null); const [busy,setBusy]=useState(false); const [sort,setSort]=useState('name');
  const selected = models.find(m=>m.id===selectedId) || null;
  const counts=useMemo(()=>TYPES.reduce((a,t)=>({...a,[t.key]:t.key==='All'?models.length:models.filter(m=>m.model_type===t.key).length}),{} as Record<string,number>),[models]);
  async function refresh(){ const s=await api.getState(); setState(s); if(!s.models_root){return;} const list=await api.listModels({type:type==='All'?undefined:type,query:query||undefined}); list.sort((a,b)=>sort==='size'?b.size_bytes-a.size_bytes:sort==='path'?a.relative_path.localeCompare(b.relative_path):(a.civitai_name||a.filename).localeCompare(b.civitai_name||b.filename)); setModels(list); if(selectedId && !list.some(m=>m.id===selectedId)) setSelectedId(list[0]?.id??null); else if(!selectedId && list[0]) setSelectedId(list[0].id); }
  useEffect(()=>{refresh();},[]);
  ...
  const doImport = async()=>{ if(!importUrl.trim()) return; setBusy(true); try {setPreview(await api.importCivitai(importUrl.trim()));} finally {setBusy(false);} };
  const install = async()=>{ if(!importUrl.trim()) return; setBusy(true); try { const m=await api.installCivitai(importUrl.trim()); setPreview(null); setImportUrl(''); await refresh(); setSelectedId(m.id); } finally { setBusy(false);} };
  return <div className="app-shell"><Background/><div className="noise"/>
    ...
    <div className="workspace">
      <aside className="sidebar hud-panel"><div className="side-title">LIBRARY</div><nav>{TYPES.map(t=><button key={t.key} className={type===t.key?'active':''} onClick={()=>setType(t.key)}><span>{t.label}</span><b>{counts[t.key]??0}</b></button>)}</nav><div className="sidebar-foot"><button className="text-btn" onClick={async()=>{const p=await api.chooseModelsFolder(); if(p) await api.setModelsRoot(p);}}>CHANGE FOLDER</button></div></aside>
      <main className="library"><div className="library-head"><div><div className="eyebrow">{type.toUpperCase()}</div><h1>{type==='All'?'MODEL LIBRARY':type.toUpperCase()}</h1></div><div className="library-tools"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search models..."/><button className="import-btn" onClick={()=>{setPreview({model:{},version:{id:0,name:'',base_model:null,download_url:'',filename:null,size_bytes:null,activation_prompts:[]},target_directory:''});}}>IMPORT CIVITAI</button><select value={sort} onChange={e=>setSort(e.target.value)}><option value="name">NAME</option><option value="size">SIZE</option><option value="path">PATH</option></select></div></div><div className="grid">{models.map(m=><ModelCard key={m.id} model={m} selected={m.id===selectedId} onClick={()=>setSelectedId(m.id)}/>)}{!models.length&&<div className="empty-state">No models match the current view.</div>}</div></main>
      {selected && <Inspector model={selected} images={images} onRefresh={async()=>{await api.refreshModel(selected.id); await refresh();}}/>}
    </div>
    {(preview || importUrl) && <div className="modal-backdrop" onClick={()=>{if(!busy){setPreview(null); setImportUrl('');}}}><div className="import-modal hud-panel" onClick={e=>e.stopPropagation()}><div className="eyebrow">CIVITAI IMPORT</div><h2>INSTALL A MODEL</h2>{!preview || preview.version.id===0 ? <div className="import-row"><input value={importUrl} onChange={e=>setImportUrl(e.target.value)} onKeyDown={e=>e.key==='Enter'&&doImport()} placeholder="https://civitai.com/models/..." autoFocus/><button className="primary-btn" onClick={doImport} disabled={busy}>{busy?'ANALYZING…':'ANALYZE'}</button></div> : <><div className="preview-panel"><h3>{preview.model.name || preview.version.filename || 'Model'}</h3><div className="kv"><span>TYPE</span><b>{preview.model.type || 'Model'}</b></div><div className="kv"><span>BASE</span><b>{preview.version.base_model || '—'}</b></div><div className="kv"><span>FILE</span><b>{preview.version.filename || 'Automatic filename'}</b></div><div className="kv"><span>TARGET</span><b className="wrap">{preview.target_directory}</b></div><div className="section-head">ACTIVATION PROMPTS</div><div className="chips">{preview.version.activation_prompts.map(x=><span key={x}>{x}</span>)}</div></div><div className="modal-actions"><button className="text-btn" onClick={()=>setPreview(null)}>BACK</button><button className="primary-btn" onClick={install} disabled={busy}>{busy?'DOWNLOADING…':'DOWNLOAD & INSTALL'}</button></div></>}</div></div>}
  </div>;
}
