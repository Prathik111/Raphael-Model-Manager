...
function App() {
  ...
  async function refresh(){ const s=await api.getState(); setState(s); if(!s.models_root){return;} const [list,allCounts]=await Promise.all([api.listModels({type:type==='All'?undefined:type,query:query||undefined}),api.getLibraryCounts()]); list.sort((a,b)=>sort==='size'?b.size_bytes-a.size_bytes:sort==='path'?a.relative_path.localeCompare(b.relative_path):(a.civitai_name||a.filename).localeCompare(b.civitai_name||b.filename)); setModels(list); setCounts(allCounts); setSelectedId(previous=>{ if(previous!==null && list.some(m=>m.id===previous)) return previous; return list[0]?.id ?? null; }); }
  useEffect(()=>{refresh();},[]);
